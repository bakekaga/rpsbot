...
class RPS(commands.Cog):
	...
	async def challenge(self,
		...
		if ctx.author == member:
			...
			return
		await ctx.channel.send(f'{ctx.author.mention} challenged {member.mention} to a game of Rock, Paper, Scissors! {rounds} game(s) will be played.')
		for round in range(rounds):
			# print(f'Playing round {round}')
			# send this message before each round to inform users about game state
			await ctx.channel.send(f'It is currently round {round + 1} out of {rounds}. {ctx.author.mention} has won {p1_wins} game(s) so far. {member.mention} has won {p2_wins} game(s) so far.')
			...
			if resp:
				...
				if ((choices[p1_choice] + 1) % 3) == choices[p2_choice]:
					await ctx.channel.send(f'{member.mention} won this round! Better luck next time {ctx.author.mention}!')
					p2_wins+= 1
				elif p1_choice == p2_choice:
					await ctx.channel.send(f'This round was a tie!')
				else:
					await ctx.channel.send(f'{ctx.author.mention} won this round! Better luck next time {member.mention}!')
					p1_wins+= 1
		# once all rounds have been played or players took too long to respond, give game summary
		await ctx.channel.send(f'The game ended with {ctx.author.mention} winning {p1_wins} rounds and {member.mention} winning {p2_wins} rounds.')
